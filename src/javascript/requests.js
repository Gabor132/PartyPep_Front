...
RequestHandler._loadClientCredentials = async function(storeState) {
  let clientCredentials = JSON.parse(
    sessionStorage.getItem("clientCredentials")
  );
...
RequestHandler._getToken = async function(username, password, storeState) {
  ...
  let clientCredentials = await RequestHandler._loadClientCredentials(
    storeState
  );
  return await axios({
    method: "post",
...
RequestHandler._loadToken = async function(storeState) {
  ...
  if (token === null) {
    ...
    token = await RequestHandler._getToken(
